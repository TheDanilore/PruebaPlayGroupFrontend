...
export default {
  ...
  methods: {
    async obtenerTamanos(page = 1) {
      try {
        const response = await axios.get(`/api/tamanos?page=${page}&per_page=${this.perPage}`)

        // Asignar las  desde la respuesta de la API
        this.tamanos = response.data.data
...
<template>
  <div class="container mt-0">
    ...
    <div class="mb-3 mt-3">
      ...
      <select v-model="perPage" @change="obtenerTamanos" id="perPage" class="form-select">
